...
export const metadata: Metadata = {
  title: "House Cleaning Blog | Tips & Guides | AlphaLux",
  description:
    "Cleaning guides, deep cleaning tips, Airbnb turnovers & move-out advice from AlphaLux. Serving TX & CA homeowners with non-toxic expertise.",
...
const SORTED_POSTS = [...BLOG_POSTS].sort((a, b) => new Date(b.publishDate).getTime() - new Date(a.publishDate).getTime());
  ...
  return (
    <>
      ...
      <section className="bg-[#0A0A0A] pt-32 pb-16 lg:pt-40 lg:pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2">
            ...
            <span>/</span>
            <span className="text-[#253858]">Blog</span>
          </nav>
          <div className="max-w-3xl">
            <p className="text-[#253858] text-xs font-semibold uppercase tracking-[0.2em] mb-3">Cleaning Guides & Tips</p>
      ...
      <section className="bg-white py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-[#253858] text-xs font-semibold uppercase tracking-[0.2em] mb-6">Featured Post</p>
          <Link href={`/blog/${featured.slug}`} className="group block bg-[#F8F7F4] border border-[#253858]/15 rounded-2xl p-8 lg:p-10 card-hover hover:border-[#253858]/30">
            ...
            <h2 className="text-[#0A0A0A] font-bold leading-snug mb-4 group-hover:text-[#253858] transition-colors" style={{ fontFamily: "var(--font-playfair)", fontSize: "clamp(1.4rem, 2.5vw, 2rem)" }}>
              {featured.title}
            </h2>
            <p className="text-gray-500 leading-relaxed mb-6 max-w-3xl">{featured.excerpt}</p>
            <span className="text-[#253858] font-medium inline-flex items-center gap-2 group-hover:gap-3 transition-all">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-10">
            ...
            <div className="flex flex-wrap gap-2">
              {CATEGORIES.map((cat) => (
                <span key={cat} className="text-[#253858] text-xs font-medium bg-[#253858]/10 border border-[#253858]/20 px-3 py-1 rounded-full">
                  {cat}
                </span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {rest.map((post) => (
              <Link key={post.slug} href={`/blog/${post.slug}`} className="group bg-white border border-[#253858]/10 rounded-xl p-6 card-hover hover:border-[#253858]/30 flex flex-col">
                ...
                <h3 className="text-[#0A0A0A] font-bold leading-snug mb-3 group-hover:text-[#253858] transition-colors flex-1" style={{ fontFamily: "var(--font-playfair)" }}>
                  {post.title}
                </h3>
                <p className="text-gray-500 text-sm leading-relaxed mb-5 line-clamp-3">{post.excerpt}</p>
                <span className="text-[#253858] text-sm font-medium inline-flex items-center gap-1 group-hover:gap-2 transition-all mt-auto">
