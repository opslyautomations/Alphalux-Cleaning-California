...
export async function generateStaticParams() {
  return Object.keys(TX_CITY_DATA).map((city) => ({ city }));
}

export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  const { city } = await params;
  const data = TX_CITY_DATA[city];
  if (!data) return {};
  return {
    title: `House Cleaning in ${data.name}, TX | AlphaLux Cleaning`,
    description: `Non-toxic house cleaning in ${data.name}, TX. Licensed & insured. Free quote. Same-week availability from AlphaLux Cleaning.`,
    alternates: { canonical: `${BASE_URL}/locations/tx/${city}` },
    openGraph: {
      type: "website",
      title: `House Cleaning in ${data.name}, TX | AlphaLux Cleaning`,
      description: `Eco-friendly house cleaning in ${data.name}, TX. Non-toxic, licensed & insured. Free quote.`,
      url: `${BASE_URL}/locations/tx/${city}`,
      siteName: "AlphaLux Cleaning",
      locale: "en_US",
      images: [{ url: `${BASE_URL}/og/location-tx-${city}.png`, secureUrl: `${BASE_URL}/og/location-tx-${city}.png`, width: 1200, height: 630, alt: `House Cleaning in ${data.name} TX — AlphaLux Cleaning`, type: "image/png" }],
    },
    twitter: { card: "summary_large_image", title: `House Cleaning in ${data.name}, TX`, description: `Eco-friendly house cleaning in ${data.name}. Non-toxic, licensed & insured.`, images: [`${BASE_URL}/og/location-tx-${city}.png`] },
    robots: { index: true, follow: true, "max-image-preview": "large", "max-snippet": -1, "max-video-preview": -1 },
    other: { "theme-color": "#253858" },
  };
}

export default async function TXCityPage({ params }: { params: Promise<{ city: string }> }) {
  const { city } = await params;
  const data = TX_CITY_DATA[city];
  if (!data) notFound();
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Texas", item: `${BASE_URL}/locations` },
      { "@type": "ListItem", position: 4, name: data.name, item: `${BASE_URL}/locations/tx/${city}` },
    ],
  };
  ...
  const localBusinessSchema = {
    ...
    name: `AlphaLux Cleaning — ${data.name}, TX`,
    url: `${BASE_URL}/locations/tx/${city}`,
    telephone: "+19725590223",
    email: "info@alphaluxclean.com",
    address: { "@type": "PostalAddress", addressLocality: data.name, addressRegion: "TX", addressCountry: "US" },
    areaServed: { "@type": "City", name: data.name, addressRegion: "TX" },
  };
      ...
      <section className="bg-[#0A0A0A] pt-32 pb-16 lg:pt-40 lg:pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2 flex-wrap">
            ...
            <span>/</span>
            <Link href="/locations" className="hover:text-[#253858] transition-colors">Locations</Link>
            <span>/</span>
          ...
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-full bg-[#253858]/15 flex items-center justify-center shrink-0">
              <MapPin className="w-6 h-6 text-[#253858]" />
            </div>
            <div>
              <p className="text-[#253858] text-xs font-semibold uppercase tracking-[0.2em] mb-2">Texas</p>
              <h1 className="text-white font-bold leading-tight mb-2" style={{ fontFamily: "var(--font-playfair)", fontSize: "clamp(1.8rem, 4vw, 3rem)" }}>
                House Cleaning in {data.name}, TX
              </h1>
              <div className="flex flex-wrap gap-4 text-sm text-gray-400 mt-3">
                ...
                <span className="flex items-center gap-1.5"><CheckCircle className="w-3.5 h-3.5 text-[#253858]" />Non-Toxic Products</span>
                <span className="flex items-center gap-1.5"><CheckCircle className="w-3.5 h-3.5 text-[#253858]" />Same-Week Availability</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-[#0A0A0A] py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-3">
            {data.nearby.map((n) => (
              <Link key={n.name} href={`/locations/${n.stateSlug}/${n.slug}`} className="bg-[#1A1A1A] border border-[#253858]/20 text-gray-300 hover:text-[#253858] hover:border-[#253858]/40 px-4 py-2 rounded-lg text-sm transition-colors flex items-center gap-2">
              ...
              <div key={r.name} className="bg-[#F8F7F4] border border-[#253858]/10 rounded-xl p-6">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-gray-500 text-xs">{data.name}, TX · {r.service}</p>
                  </div>
      ...
      <section className="bg-[#0A0A0A] py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <div>
              <p className="text-[#253858] text-xs font-semibold uppercase tracking-[0.2em] mb-3">Same-Week Availability</p>
              ...
              <div className="flex flex-col gap-2 text-sm text-gray-400">
                <span className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-[#253858]" />Licensed &amp; Insured in Texas</span>
                <span className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-[#253858]" />100% Non-Toxic Products</span>
                <span className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-[#253858]" />Background-Checked Teams</span>
                <span className="flex items-center gap-2"><Clock className="w-4 h-4 text-[#253858]" />Mon–Sat 8:00 AM – 6:00 PM</span>
              </div>
            </div>
            <div className="bg-[#1A1A1A] border border-[#253858]/20 rounded-2xl p-6">
              ...
              <p className="text-gray-500 text-sm mb-5">Serving {data.name}, TX · Response within 24 hours.</p>
              <GHLForm />
            </div>
          </div>
        </div>
