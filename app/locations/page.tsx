      ...
      <section className="bg-[#0A0A0A] pt-32 pb-16 lg:pt-40 lg:pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2">
            ...
            <span>/</span>
            <span className="text-[#253858]">Locations</span>
          </nav>
          <div className="max-w-3xl">
            <p className="text-[#253858] text-xs font-semibold uppercase tracking-[0.2em] mb-3">Where We Serve</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-12">
            <div>
              <div className="flex items-center gap-3 mb-6">
                <span className="w-8 h-8 rounded-full bg-[#253858] text-[#0A0A0A] text-sm font-bold flex items-center justify-center">TX</span>
                <h2 className="text-[#0A0A0A] font-bold text-xl" style={{ fontFamily: "var(--font-playfair)" }}>Texas</h2>
              </div>
              <div className="space-y-3">
                {TX_CITIES.map((c) => (
                  <Link key={c.slug} href={`/locations/tx/${c.slug}`} className="group flex items-center justify-between bg-[#F8F7F4] border border-[#253858]/10 rounded-xl p-4 hover:border-[#253858]/30 transition-colors card-hover">
                    <div className="flex items-center gap-3">
                      <MapPin className="w-4 h-4 text-[#253858]" />
                      <span className="font-medium text-[#0A0A0A] group-hover:text-[#253858] transition-colors">House Cleaning in {c.name}, TX</span>
                    </div>
                    <ArrowRight className="w-4 h-4 text-[#253858]" />
                  </Link>
                ))}
              </div>
            </div>
            <div>
              <div className="flex items-center gap-3 mb-6">
                <span className="w-8 h-8 rounded-full bg-[#253858] text-[#0A0A0A] text-sm font-bold flex items-center justify-center">CA</span>
                <h2 className="text-[#0A0A0A] font-bold text-xl" style={{ fontFamily: "var(--font-playfair)" }}>California</h2>
              </div>
              <div className="space-y-3">
                {CA_CITIES.map((c) => (
                  <Link key={c.slug} href={`/locations/ca/${c.slug}`} className="group flex items-center justify-between bg-[#F8F7F4] border border-[#253858]/10 rounded-xl p-4 hover:border-[#253858]/30 transition-colors card-hover">
                    <div className="flex items-center gap-3">
                      <MapPin className="w-4 h-4 text-[#253858]" />
                      <span className="font-medium text-[#0A0A0A] group-hover:text-[#253858] transition-colors">House Cleaning in {c.name}, CA</span>
                    </div>
                    <ArrowRight className="w-4 h-4 text-[#253858]" />
                  </Link>
                ))}
          ...
          <div className="mt-12 text-center">
            <p className="text-gray-500 text-sm">
              Don&apos;t see your city?{" "}
              <a href={PHONE_TEL} className="text-[#253858] hover:underline font-medium">Call {PHONE_DISPLAY}</a>
